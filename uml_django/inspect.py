...
class Diagram:
    def __init__(self, class_list, url):
        ...
        self.url = url
        self.uml = PlantUML(self.url)

    def generate_image(self, clss):
        return self.uml.processes(clss)
    ...
    def create_diagram(self, classes):
        ...
        image = self.generate_image(classes)

        self.generate_png(image)
                        ...
                        if isinstance(member, ast.FunctionDef):
                            ...
                            if isinstance(member.value, ast.Call):
                                if member.value.func.attr == "ForeignKey":
                                    classDict["oneToMany"].append(
                                        member.value.args[0].id
                                    )
                                elif member.value.func.attr == "ManyToManyField":
                                    classDict["manyToMany"].append(
                                        member.value.args[0].id
                                    )
